...
## ⚠️ Landmine: `npx cap sync ios` / `npx cap update ios` will silently
## un-register `StreakBridgePlugin` — re-add it to `packageClassList` after
...
Fixed by manually adding `"StreakBridgePlugin"` to `packageClassList` in
`ios/App/App/capacitor.config.json`. **The catch**: that array is
auto-generated by the Capacitor CLI (`iosplugin.js`'s `findPluginClasses`)
by scanning `.swift`/`.m` files inside *installed npm Capacitor plugin
packages* (`@capacitor/filesystem`, `@capacitor/local-notifications`,
`@capacitor/preferences`, `@capacitor/share`) — it has no way to see
`StreakBridgePlugin.swift`, since that's a hand-written file living
directly in the Xcode project, not an npm package. `npx cap sync ios` (or
`cap update ios`) **overwrites the whole array** with only what it finds
from that npm scan, silently dropping `StreakBridgePlugin` again. After
running either of those commands for iOS, re-check
`ios/App/App/capacitor.config.json` and re-add `"StreakBridgePlugin"` to
`packageClassList` if it's missing — nothing will error, the widget will
just quietly stop getting streak updates again, exactly like this bug.
