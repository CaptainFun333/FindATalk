...
custom native bridge (`StreakBridgePlugin`, packaged at
`packages/capacitor-streak-bridge/` — see the landmine writeup below for
why it isn't a bare file in `ios/App/App/` anymore), which needs
...
## ✅ Fixed 2026-09-09: the `npx cap sync ios` landmine that kept stripping
## `StreakBridgePlugin` from `packageClassList`

Found 2026-09-01 debugging "streak not showing in widget" even after App
Groups was correctly enabled on both targets (above). Root cause:
`ios/App/App/capacitor.config.json`'s `packageClassList` array is what
Capacitor's iOS bridge actually registers with the JS side
(`CapacitorBridge.swift`'s `registerPlugins()` calls
`NSClassFromString(name)` for each entry) — a plugin not listed there is
just invisible to `window.Capacitor.Plugins`, no matter how correct its
Swift code or entitlements are. `StreakBridgePlugin` had never been added
to this list, so `cap.Plugins.StreakBridge` was `undefined` in the WebView
the entire time; `mirrorStreakToNative()`'s iOS branch in `docs/index.html`
was silently a no-op (caught by its own `try/catch`, so nothing errored).
Confirmed via the simulator's own filesystem: the App Group container
existed (created just by the app launching with the entitlement) but its
`Library/Preferences` was completely empty — proof no write had ever
reached it.

Patched at the time by manually adding `"StreakBridgePlugin"` back to
`packageClassList` — but that array is auto-generated by the Capacitor
CLI (`iosplugin.js`'s `findPluginClasses`) by scanning `.swift`/`.m`
files inside *installed npm Capacitor plugin packages*
(`@capacitor/filesystem`, `@capacitor/local-notifications`,
`@capacitor/preferences`, `@capacitor/share`) — it had no way to see
`StreakBridgePlugin.swift`, since that was a hand-written file living
directly in the Xcode project, not an npm package. Every `npx cap sync
ios` (or `cap update ios`) **overwrote the whole array** with only what
it found from that npm scan, silently dropping the manual entry again —
this happened for real at least twice after the original fix, most
recently mid-September while building an unrelated iOS feature.

**Permanent fix (2026-09-09)**: repackaged `StreakBridgePlugin` as its
own tiny local Capacitor plugin instead of a bare file in the Xcode
project, so the CLI's scan actually discovers it like any other plugin
and stops overwriting it:
- New package at `packages/capacitor-streak-bridge/` — a real (if
  unpublished) Capacitor plugin: `package.json` with a `"capacitor":
  {"ios": {"src": "ios"}}` manifest, its own `Package.swift` (product
  name `CapacitorStreakBridge`, matching the CLI's `fixName()`-derived
  name for the npm package name `capacitor-streak-bridge`), the actual
  plugin source moved to `ios/Sources/StreakBridgePlugin/
  StreakBridgePlugin.swift` (content unchanged), and a minimal JS
  surface (`dist/esm/index.js` + a `web.js` no-op fallback, `dist/
  plugin.cjs.js`) that nothing in this app actually imports —
  `docs/index.html` still calls it dynamically via
  `Capacitor.Plugins.StreakBridge.*`, the same as before — but which
  keeps the package a correct, complete plugin rather than a bare
  Swift-only shim.
- Root `package.json` depends on it via `"capacitor-streak-bridge":
  "file:packages/capacitor-streak-bridge"` (a symlink into
  `node_modules`, same as any other local npm package — `npm install`
  after cloning sets this up automatically, nothing extra to do).
- `ios/App/App/StreakBridgePlugin.swift` was deleted, and its three
  references in `ios/App/App.xcodeproj/project.pbxproj` (PBXBuildFile,
  PBXFileReference, group child, Sources build phase) removed by hand —
  this project's Xcode file list is explicit references, not a
  filesystem-synchronized group, so a moved/deleted file needs that
  manual cleanup or the build fails looking for a missing path.
- Now `npx cap sync ios` (and `cap update ios`) find `capacitor-streak-
  bridge` in `node_modules` (via the CLI's own dependency scan of root
  `package.json`), auto-add `"StreakBridgePlugin"` to `packageClassList`,
  and auto-add the package as a dependency inside
  `ios/App/CapApp-SPM/Package.swift` (the nested local SPM package the
  Xcode project links against as a whole — no `project.pbxproj` edit
  needed for that part, since it's just one more dependency of the
  single `CapApp-SPM` product Xcode already depends on). Verified live:
  ran `npm install` + `npx cap sync ios`, confirmed both files
  regenerated with the plugin present, did a clean `xcodebuild` (0
  errors/warnings), then confirmed at runtime in the Simulator that
  toggling Light/Dark in Settings still reaches
  `StreakBridgePlugin.setThemePreference` (temporary `NSLog` + `log
  stream`, then reverted). `npx cap sync android` confirmed unaffected —
  the plugin has no `android` key in its manifest, so Android's plugin
  scan silently skips it.
- This is a one-time restructuring, not an ongoing maintenance item —
  from here on, `cap sync`/`cap update` on iOS should just keep working
  without ever needing `packageClassList` hand-edited again. If a
  *second* app-only native plugin is ever added, package it the same
  way (a folder under `packages/`, `file:` dependency in
  `package.json`) rather than dropping a bare `.swift` file into
  `ios/App/App/` — that bare-file shortcut is exactly what caused this
  whole recurring bug.
