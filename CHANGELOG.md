...
- The "Come, Follow Me" tab now has the same Topic/Calling/Speaker/Conference/Session filters as the home page, a Most Matching/Most Recent sort, and pages through results 10 at a time — with "Select First 10"/"Select All" to add a whole page (or every match) to a list at once, same as "Show a List" already offers. Handy on a week with a lot of matches: filter down to just talks by a Calling you care about, then sort by whichever is more useful.

### Changed
- On the "Come, Follow Me" tab, the date range is now bigger and set apart from the explanation text below it, and that explanation is now a few short lines instead of one long sentence.
