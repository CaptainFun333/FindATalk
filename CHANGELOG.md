...
- New "Your FindATalk" window shows your reading numbers all in one place — unique talks read (or talks remaining — tap the number to flip it), current or longest streak (same tap-to-flip), days active in the last year, total reads including rereads, and your most-read talk — plus how many badges you've earned and a way to support the app. Open it from the new trophy button next to Settings, or tap your streak pill on the Home screen. (Requires a new build on both iPhone/iPad and Android to take effect — live immediately on the website.)
...
- The website now shows the pulpit app icon as its browser tab icon (favicon), instead of a generic blank tab. (Live immediately on the website; doesn't apply to the native apps, which have no browser tab to show it in.)

### Changed
- Moved My Badges and Support the App out of Settings into their own new button next to it — tap the trophy icon in the top-left to see your badges, stats, and ways to support the app all in one place, without digging through Settings. Settings itself is unchanged otherwise. (Requires a new build on both iPhone/iPad and Android to take effect — live immediately on the website.)
...
- "New list name…" and Create in the Add to a List popup now always stay visible at the bottom instead of scrolling out of view once you have a lot of lists — same fix keeps Cancel/Save/Share reachable in My Note and Share This Talk even if you drag their message box taller. (Requires a new build on both iPhone/iPad and Android to take effect — live immediately on the website.)
...
- App-exclusive (iOS): signing in with Apple or Google never actually synced anything to the cloud — favorites, notes, lists, and streak stayed stuck on that one device, and the tip jar's purchase confirmation silently failed too, even though Settings said you were signed in. Both now work correctly. Requires a new build on both iPhone/iPad and Android to take effect.
...
- App-exclusive (Android and iOS): once cloud sync was actually working (see the fix above), a bug in how streak data merges could get sync stuck endlessly re-syncing the same data in the background — which could also make buttons elsewhere in the app (like "Support the App" in Settings) seem unresponsive while it was happening. Fixed. Requires a new build on both platforms to take effect.
- App-exclusive (iOS): fixed the tip jar pointing at retired product identifiers after they were recreated on the App Store side — tips would have failed to purchase. Requires a new iOS build to take effect.
- App-exclusive (Android): upgraded the Play Billing library to the version Google now requires for new submissions — Play Console was rejecting the app bundle without this. Requires a new Android build to take effect.
- App-exclusive (Android): updated the tip jar to Play Console's new lowercase-only product identifiers after the originals were recreated to satisfy that requirement. Requires a new Android build to take effect.
...
- Favoriting a talk (or adding a note) on one signed-in device while another signed-in device saved to the cloud around the same time could silently erase the first device's addition instead of combining them — cloud sync now always merges in the latest cloud data before saving, so simultaneous changes on different devices are combined instead of one overwriting the other.
...
- Renamed the "Find Another" and "Show a List" buttons on Home to "Find a Talk" and "Show Matches" — same buttons, same behavior, just clearer wording.
- A little easter egg hidden in the app is now a bit easier to stumble onto.
...
- A List's own page now shows its talk count and the Read/Unread toggle at the top in the same style as Home and Come, Follow Me, with "Search this list" moved inside that box; removed the separate "Showing all N talks" line since the count above already covers it.

## [1.5.4] — 2026-09-05

### Fixed
- Android: fixed shared findatalk.com links (a talk, streak, badge, or list) opening in the browser instead of the app on phones that installed the app from Play Store — Play App Signing re-signs the app with a certificate that wasn't yet listed as trusted.
...
- Fixed sharing a talk, your streak, a badge, or a list from a Chrome browser (not the app) still showing the link twice in the share message — the earlier fix only covered the app's own share sheet, not the browser's.
